#!/usr/bin/env python3
"""
WSR Daily Orchestrator — MikLin/Mulligan Acquisition Stores
...
Profile: mm (MikLin/Mulligan)
Credentials: MM_SITE_USERNAME / MM_SITE_PASSWORD
Expected stores: MM_EXPECTED_STORES (set in GitHub Secrets)
Download dir: wsr_downloads_mm/
"""
...
# module-level globals in wsr_download_acquisition resolve to MM credentials.
# ---------------------------------------------------------------------------
...
EXPECTED_STORES   = 8  # MikLin/Mulligan group
...
class WSROrchestrator:
    ...
    async def run_download_for_week(self, week: int, year: int, week_label: str) -> bool:
        ...
        try:
            # Import acquisition downloader (WSR_PROFILE already set to 'mm')
...
async def main():
    ...
    if EXPECTED_STORES == 0:
        print(f"{Colors.YELLOW}⚠ MM_EXPECTED_STORES is not set — audit comparisons will flag every week{Colors.RESET}")
